...
* Администратор будет иметь возможность создать аккаунт новому менеджеру
...
- Простой, интуитивный интерфейс, позволяющий начать работу с системой без специального обучения
...
##### Главная страница
- После прохождения процедуры авторизации пользователь попадает на главное окно, отображающую текущую занятость мастеров. В верхней части страницы имеется панель с возможностью перейти к отслеживанию работы над своим автомобилем.
...
- В верхней панели появится возможность добавления нового менеджера.
...
Менеджер
-->
- Открыть список управляемых заказов
...
- Зарегестрировать новго менеджера
- Открыть список занятости всех менеджеров
- Сменить менеджера для заказа
<!--
...
- При регистрации на занятое время
- При попытке написать в завершенный чат( надо ли? )
- Неавторизированные пользователи не могут просматривать свои заказы и прочее
- Ограниченный функционал в соответствии с уровнем доступа
- При потери связи с бд, должно выводится сообщение о тех. неполадках
- Бекап БД ( опять же, надо ли? )
- Регистрация на уже зареганный логин/email
- Проверка email на валидность ( *@*.* )
<!--
Для каждой возможной ошибки в работе необходимо прописать
ID
Принадлежность к группе
Описание ( при нажатии не туда, вылезает ошибка такая-то
Приоритет
Риск
Связанные задачи
-->
##### Требования производительности
- Тут всё просто: При стабильной связи с интернетом, отклик не более 5 сек
- Можно разбить на запись( отклик до 3-х секунд ) и вывод БД ( отклик до 10 сек )
<!--
Прописать общие тех.характеристики, которые может гарантировать наш код
ID
Принадлежность к группе
Описание ( такая-то операция занимает столько то времени )
Приоритет
Риск
Связанные задачи
-->
##### Требования удобства использования
- Фильтрация нинужна( хотя её реализовать с помощью бд не сложно )
- Интуитивный интерфейс
- Возможность из любого окна переместится в любое другое доступное окно
- Выход из системы ( да, да, да, ни на одной из схем и требований этого нет (( )
<!--
Для каждой функциональной задачи необходимо прописать
ID
Принадлежность к группе
Описание ( херь по типу, интуитивный интерфейс, фильтрация, для упрощения поиска и тд. )
Приоритет
Риск
Связанные задачи
-->
