...
-->
Зависимости Системы:
* Системе необходим стационарный ПК.
На нём должна быть операционная система Windows 7,8,10.
* Сеть.
ПК должен быть подключён к сети, скорость подключения не меньше 8 МБит/сек.
* Процессор.
На ПК должен быть минимум 4-х ядерный процессор с такстовой частотой ядер не меньше 1.3 Ггц.
* Нагрузка.
ПК должен работать непрерывно в течении 6ти рабочих дней. На нём не должно быть установлено ничего, помимо стандартный приложений Windows и дополнительных программ, необходимых для работы системы. Для этого нельзя допускать перегревов, повышенной влажности в помещении с этим ПК. Необходима переодическая прочистка от пыли как в самом помещении, так и внутри ПК.
* Жесткий диск и оперативная память.
Для работы системы необходим жесткий диск ПК минимум на 256Гб. Объём оперативной памяти не менее 4 Гб.
* Программное обеспечение.
На ПК необходима установленная PostgreSQL.

<!-- Пока что это всё, если есть ещё идеи - пишите -->
