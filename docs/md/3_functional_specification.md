...
![5.1п](images/5.1п.PNG)

Функционал, изображенный:
- Зеленым цветом - доступен всем авторизированным пользователям.
- Желтым цветом - доступен только менеджерам и администраторам.
- Красным цветом - доступен только администратору.
...
-->
Логический проект решения, включающий компоненты программного решения, их взаимосвязь между собой, а также используемые технологии представлен на диаграмме:

![5.2п](images/5.2п.PNG)

Интерфейс будет включать в себя несколько страниц, каждая из которых представляет свою функциональность:
...
-->
Архитектура серверной части приложения состоит из трех основных частей:

![5.3п](images/5.3п.PNG)

База данных (в проекте используется СУБД PostgreSQL) осуществляет хранение данных, необходимых приложению в процессе работы. Ниже приведена диаграмма объектов базы данных и их взаимосвязь между собой:
...
-->
Для инсталляции решения необходимо оборудование с установленными ОС Windows и Java Virtual Machine.
